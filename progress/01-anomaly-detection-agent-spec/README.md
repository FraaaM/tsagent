...
**Leak-free sampling (v2).** No chunk boundary ever depends on the labels: the chunk size
...
(cap 0.30) is enforced purely by rejecting over-budget slots. Under the legacy v1 logic
— dilution and cluster expansion stretched exactly the dirty samples — a length-only
classifier scored 0.97 AUROC on R2; under v2 the same probe scores 0.53 on R2 and 0.64 on
...
`SAMPLER_LOGIC = "v1"` in the sampler to reproduce the old pools byte-for-byte.
...
**Enriched synthetic regime (v2).** Heavy-tailed (Student-t) innovations on ~25% of
...
beyond 3.5σ (v2.5): tails mean frequent 2.5–3.5σ shoulders — hard negatives — not lone
