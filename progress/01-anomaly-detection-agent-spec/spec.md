...
- **Base-process realism (generation logic v2)** — the "normal" regime is deliberately harder than textbook processes, so detectors cannot pass by assuming iid Gaussian noise:
...
- **Series ID**: `{group}__{original_dataset}__{original_id}_{sample_id}`
  ...
  - `sample_id`:
    ...
    - `"clean{N}"` - legacy v1 sampling only (see below); does not occur in v2 pools
  ...
  **Splitting rule (v2, applied once during pool creation, see `src/sampler-r1-r2.py`)**:
  ...
  **Anomaly-ratio budget (v2)**: a positive series should *contain* an anomaly, not consist of one. A single cap `V2_MAX_ANOMALY_RATIO = 0.30` is enforced **purely by rejection**: an over-budget slot is dropped, and the grid never moves in response. Above ~0.30 the label "series contains an anomaly" stops being an honest description and majority-based detectors start inverting; below the cap the natural per-chunk distribution is preserved untouched.

  **Representativeness screen (v2, label-noise guard)**: a *clean-labelled* chunk wildly unlike its parent series is suspicious of unlabelled anomalies (known TSB-UAD label noise) and is dropped. The deviation of the chunk's raw mean/std is measured against a **robust reference** — the parent's statistics after winsorizing at 5 robust σ (`V2_REFERENCE_CLIP_SIGMAS`), so a recording's own anomalies cannot widen its tolerance band — with threshold `V2_STATS_TOLERANCE = 1.5`, placed where measured innocence ends: clean R1 chunks deviating 1.0–1.5 still look like the bulk (max excursion ≈ 3σ), while those beyond 1.5 carry unlabelled excursions of 9–44σ. The screen costs 4.2% of R1's clean slots and 0.7% of R2's.

  **Leak-freedom (the reason v2 exists)**: no sample boundary ever depends on the labels, so sample length cannot act as a proxy for `y_i`. The legacy v1 logic (dilution toward a 0.06 target, anomaly-cluster expansion, whole-series triage, `clean{N}` salvage) stretched exactly the anomalous samples: a length-only classifier reached **0.97 AUROC on R2**, and 88.7% of dirty R2 samples sat pinned to a dilution threshold. Under v2 the same probe scores 0.53 on R2 and 0.64 on R1 (the residual is the statistical fact that a longer window is likelier to contain an anomaly — the price of keeping realistic, period-honest per-series lengths), pinning is < 1%, and the mean length of positive vs negative samples is 1.00x on R2 (was 3.90x). v1 remains available via `SAMPLER_LOGIC = "v1"` and reproduces the old pools byte-for-byte.
  - Realised ratio maxima (v2): R1 = 0.299, R2 = 0.299, S1 = 0.198, S2 = 0.299 (the S2 tail above 0.20 comes from the periodic duration floors — events on seasonal bases must span ≥ 2 full periods — which can exceed the drawn fraction on short series; everything stays under the 0.30 budget shared with the real pools).
...
- **Metadata**: A separate small parquet file `{group}_metadata.parquet` (one per group) with the following columns:
  ...
  - `is_split` (boolean)
  - `original_length`
  - `source_notes` (free-form provenance tags: `period=…`; v1 legacy pools may also carry `whole_series`, `clean_salvage`, `tail_merged`)
...
- An anomaly cluster falling on a grid boundary may split into two positive chunks, each holding part of the cluster. This is accepted: moving boundaries to avoid it would tie the grid to the labels, which is exactly the leak v2 removes.
